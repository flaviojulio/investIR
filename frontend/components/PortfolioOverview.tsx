...
import type { Operacao, CarteiraItem, ResultadoMensal, OperacaoFechada } from "@/lib/types"

interface PortfolioOverviewProps {
  carteira: CarteiraItem[]
  resultados: ResultadoMensal[]
  operacoes: Operacao[]
  operacoesFechadas: OperacaoFechada[] // NOVA PROP para operações encerradas
  totalDividendosRecebidos: number // NOVA PROP
  showValues: boolean // New prop for hiding/showing values
}

export function PortfolioOverview({ carteira, resultados, operacoes, operacoesFechadas, totalDividendosRecebidos, showValues }: PortfolioOverviewProps) {
      ...
      <div className="bg-gradient-to-br from-orange-50 to-orange-100 border-2 border-orange-200 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105">
        <div className="p-6">
          ...
          <div className="text-3xl font-bold text-orange-900 mb-2">
            {formatCurrency(metrics.resultadoTotal)}
          </div>
          <p className="text-sm text-orange-600">
            {metrics.resultadoTotal >= 0 ? 'Lucro em operações encerradas' : 'Prejuízo em operações encerradas'}
          </p>
        </div>
      </div>
    </div>
