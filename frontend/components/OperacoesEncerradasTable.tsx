...
const getDarfStatusForOperation = (
  ...
  console.log("🔍 [DARF STATUS] Verificando chave:", {
    operationKey,
    hasInMap: darfStatusMap?.has(operationKey),
    mapSize: darfStatusMap?.size,
    allKeys: darfStatusMap ? Array.from(darfStatusMap.keys()) : [],
  });

  // Verifica se há status específico no mapa
  if (darfStatusMap && darfStatusMap.has(operationKey)) {
    const status = darfStatusMap.get(operationKey);
    console.log("🎯 [DARF STATUS] Status encontrado:", status);
    return status;
  }

  // Lógica padrão: assumir pendente para operações tributáveis
  console.log("📋 [DARF STATUS] Usando status padrão: pendente");
  return "pendente";
};
      ...
      >
        ...
        <div className="col-span-3 flex items-center justify-start">
          <div className="flex items-center gap-2 min-h-[32px]">
            ...
            {(op.status_ir === "Tributável Day Trade" ||
              op.status_ir === "Tributável Swing") &&
              getDarfBadge(getDarfStatusForOperation(op, darfStatusMap), op)}
          </div>
        </div>
      </div>
...
export default function OperacoesEncerradasTable(
  ...
  // Forçar re-renderização após atualização do status do DARF
  const [, forceUpdate] = useState({});

  // Handler for DARF status change
  // Atualiza o status do DARF para todas as operações do mesmo mês e tipo
  const handleDarfStatusChange = (operationKey: string, newStatus: string) => {
    ...
    // Extrai dados da operação selecionada
    const opRef = operacoesFechadas.find(op => {
      const key = `${op.ticker}-${op.data_abertura}-${op.data_fechamento}-${op.quantidade}`;
      return key === operationKey;
    });
    if (!opRef) {
      console.warn("Operação não encontrada para atualização em lote do DARF.");
      return;
    }
    const mes = opRef.data_fechamento.substring(0, 7);
    const isDayTrade = opRef.day_trade;

    setDarfStatusMap((prev) => {
      const updated = new Map(prev);
      // Atualiza todas as operações do mesmo mês e tipo
      operacoesFechadas.forEach(op => {
        const opMes = op.data_fechamento.substring(0, 7);
        const opIsDayTrade = op.day_trade;
        if (opMes === mes && opIsDayTrade === isDayTrade) {
          const key = `${op.ticker}-${op.data_abertura}-${op.data_fechamento}-${op.quantidade}`;
          updated.set(key, newStatus);
        }
      });
      console.log("🔄 [DARF STATUS] Map atualizado em lote:", Array.from(updated.entries()));
      return updated;
    });
    setIsDarfModalOpen(false); // Optionally close modal after status change
    // Forçar re-render para garantir atualização do badge
  ...
  const getDarfBadge = (darfStatus: string | null, op: OperacaoFechada) => {
    ...
    return (
      <span
        className={`px-2 py-1 rounded-full text-xs font-semibold border border-${color}-300 ${bg} text-${color}-700 ml-1`}
      >
        {text}
      </span>
    );
  ...
  const getStatusBadge = (status: string, isProfit: boolean) => {
    ...
    if (status === "Isento") {
      ...
      status === "Tributável Day Trade" ||
      status === "Tributável Swing"
    ) {
      color = "blue";
      bg = "bg-blue-100";
      text = "Tributável";
  ...
  const processedOperacoes = useMemo(() => {
    ...
    if (filterMonth !== "all") {
      ops = ops.filter(
        (op) => op.data_fechamento.substring(0, 7) === filterMonth
      );
    }
    // Filter by status
    if (filterStatus !== "all") {
    ...
    <>
        ...
        <CardContent className="p-6">
          ...
          <div className="border border-indigo-200 border-t-0 rounded-b-xl overflow-hidden">
            {sortedOperacoes.map((op, index) => (
              <OperationRow
                key={
                  op.id ? op.id : `${op.ticker}-${op.data_fechamento}-${index}`
                }
                op={op}
                index={index}
      ...
      {isDarfModalOpen && selectedOpForDarf && (
        <DarfDetailsModal
          ...
          // Passa operationKey explicitamente
          onDarfStatusChange={(newStatus) => {
            ...
            handleDarfStatusChange(operationKey, newStatus);
          }}
          operacoesFechadas={operacoesFechadas}
        />
      )}
