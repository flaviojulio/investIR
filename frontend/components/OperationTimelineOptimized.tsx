...
type FilterKey = "all" | "buy" | "sell" | "proventos" | "events" | "fechadas";
...
const getColorScheme = (operation: string) => {
...
const getSellIcon = (resultado?: number) => {
...
const FilterButton = memo<FilterButtonProps>(({ filterKey, config, isActive, onClick, count = 0 }) => {
  const handleClick = useCallback(() => onClick(filterKey), [filterKey, onClick]);
...
// Componente para operação individual memoizado
interface OperationCardProps {
  item: any;
  idx: number;
}

const OperationCard = memo<OperationCardProps>(({ item, idx }) => {
  ...
  return (
    <motion.div
      ...
      transition={{ 
        delay: Math.min(idx * 0.02, 0.5), // Reduzir delay máximo para melhor performance
        type: "spring",
        stiffness: 400,
        damping: 25
      }}
          ...
          <div className="space-y-1">
            ...
            {!isProvento && !isEventoCorporativo && displayQuantity && displayPrice && (
              <div className="text-right">
                ...
                {(operation === 'sell' || operation === 'fechamento') && item.resultado !== undefined && (
...
const OperationTimelineOptimized = memo<Props>(({ items = [] }) => {
  const [activeFilter, setActiveFilter] = useState<FilterKey>("all");
  const [searchTerm, setSearchTerm] = useState("");

  // Filtrar itens pela busca primeiro - otimizado
  const searchFilteredItems = useMemo(() => {
    ...
    return items.filter(item => {
      ...
      return ticker.toLowerCase().includes(searchLower) || 
             nomeAcao.toLowerCase().includes(searchLower);
    });
  }, [items, searchTerm]);

  // Filtrar por categoria - otimizado
  const filteredItems = useMemo(() => {
    if (activeFilter === "all") return searchFilteredItems;
  ...
  const filterCounts = useMemo(() => {
    ...
    searchFilteredItems.forEach(item => {
      ...
      if (operation === 'fechamento') counts.fechadas++;
      if (FILTER_CONFIG.buy.types?.includes(operation)) counts.buy++;
      if (FILTER_CONFIG.sell.types?.includes(operation)) counts.sell++;
      if (FILTER_CONFIG.proventos.types?.includes(operation)) counts.proventos++;
      if (FILTER_CONFIG.events.types?.includes(operation)) counts.events++;
    });
  ...
  // Items com distribuição automática no lado - otimizado
  const distributedItems = useMemo(() => {
    return filteredItems.map((item, index) => ({
      ...item,
      visualBranch: ['dividend', 'jcp', 'rendimento'].includes(item.operation) 
        ? 'right' 
        ...
        <div className="relative">
          ...
          <div className="space-y-8">
            {distributedItems.map((item, idx) => (
              <OperationCard key={`${item.id}-${idx}`} item={item} idx={idx} />
            ))}
          </div>
...
OperationTimelineOptimized.displayName = 'OperationTimelineOptimized';

export default OperationTimelineOptimized;