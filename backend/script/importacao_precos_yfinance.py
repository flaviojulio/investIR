    ...
    def baixar_cotacoes_acao(self, acao_id, ticker, meses=12):
        ...
        try:
            ...
            df_cotacoes = pd.DataFrame({
                ...
                'abertura': hist['Open'].round(2),
                'maxima': hist['High'].round(2),
                'minima': hist['Low'].round(2),
                'fechamento': hist['Close'].round(2),
                'fechamento_ajustado': hist['Adj Close'].round(2),
                'volume': hist['Volume'].astype('int64'),
                'dividendos': hist['Dividends'].round(4),
                'splits': hist['Stock Splits'].round(4)
            })
            ...
            logger.error(f"Erro ao baixar dados de {ticker} (ID: {acao_id}): {e}")
            return None
